...
/// <list type="bullet">
///   <item>During the visit of each instruction in MethodWriter, the state of the frame at the end of
///       the current basic block is updated by simulating the action of the instruction on the
///       previous state of this so called "output frame".</item>
///   <item>After all instructions have been visited, a fix point algorithm is used in MethodWriter to
///       compute the "input frame" of each basic block (i.e. the stack map frame at the beginning of
///       the basic block). See <c>MethodWriter.computeAllFrames</c>.</item>
/// </list>
/// </para>
/// <para>
...
/// type at position x from the top of the input frame stack" or even "the type at position x in the
/// input frame, with y more (or less) array dimensions". This explains the rather complicated type
/// format used in this class, explained below.
/// </para>
/// <para>
...
/// </para>
/// <list type="bullet">
///   <item>the DIM field, stored in the 6 most significant bits, is a signed number of array
///       dimensions (from -32 to 31, included). It can be retrieved with <see cref="DIM_MASK"/> and a
///       right shift of <see cref="DIM_SHIFT"/>.</item>
///   <item>the KIND field, stored in 4 bits, indicates the kind of VALUE used. These 4 bits can be
///       retrieved with <see cref="KIND_MASK"/> and, without any shift, must be equal to
///       <see cref="CONSTANT_KIND"/>, <see cref="REFERENCE_KIND"/>, <see cref="UNINITIALIZED_KIND"/>,
///       <see cref="FORWARD_UNINITIALIZED_KIND"/>, <see cref="LOCAL_KIND"/> or <see cref="STACK_KIND"/>.</item>
///   <item>the FLAGS field, stored in 2 bits, contains up to 2 boolean flags. Currently only one flag
///       is defined, namely <see cref="TOP_IF_LONG_OR_DOUBLE_FLAG"/>.</item>
///   <item>the VALUE field, stored in the remaining 20 bits, contains either
///       <list type="bullet">
///         <item>one of the constants <see cref="ITEM_TOP"/>, <see cref="ITEM_ASM_BOOLEAN"/>,
///             <see cref="ITEM_ASM_BYTE"/>, <see cref="ITEM_ASM_CHAR"/> or <see cref="ITEM_ASM_SHORT"/>,
///             <see cref="ITEM_INTEGER"/>, <see cref="ITEM_FLOAT"/>, <see cref="ITEM_LONG"/>,
///             <see cref="ITEM_DOUBLE"/>, <see cref="ITEM_NULL"/> or <see cref="ITEM_UNINITIALIZED_THIS"/>,
///             if KIND is equal to <see cref="CONSTANT_KIND"/>.</item>
///         <item>the index of a <see cref="Symbol.CONSTANT_CLASS_TAG"/> <see cref="Symbol"/> in the type
///             table of a <see cref="SymbolTable"/>, if KIND is equal to <see cref="REFERENCE_KIND"/>.</item>
///         <item>the index of an <see cref="Symbol.UNINITIALIZED_TYPE_TAG"/> <see cref="Symbol"/> in the
///             type table of a <see cref="SymbolTable"/>, if KIND is equal to
///             <see cref="UNINITIALIZED_KIND"/>.</item>
///         <item>the index of an <see cref="Symbol.FORWARD_UNINITIALIZED_TYPE_TAG"/> <see cref="Symbol"/>
///             in the type table of a <see cref="SymbolTable"/>, if KIND is equal to
///             <see cref="FORWARD_UNINITIALIZED_KIND"/>.</item>
///         <item>the index of a local variable in the input stack frame, if KIND is equal to
///             <see cref="LOCAL_KIND"/>.</item>
///         <item>a position relatively to the top of the stack of the input stack frame, if KIND is
///             equal to <see cref="STACK_KIND"/>.</item>
///       </list>
///   </item>
/// </list>
/// <para>
...
/// UNINITIALIZED_KIND or FORWARD_UNINITIALIZED_KIND abstract types of positive or <c>null</c>
...
internal sealed class Frame
{
    ...
    /// <summary>The bitmask to extract the DIM field of an abstract type.</summary>
    private const int DIM_MASK = ((1 << DIM_SIZE) - 1) << DIM_SHIFT;

    /// <summary>The bitmask to extract the KIND field of an abstract type.</summary>
    private const int KIND_MASK = ((1 << KIND_SIZE) - 1) << KIND_SHIFT;

    /// <summary>The bitmask to extract the VALUE field of an abstract type.</summary>
    private const int VALUE_MASK = (1 << VALUE_SIZE) - 1;

    // Constants to manipulate the DIM field of an abstract type.

    /// <summary>The constant to be added to an abstract type to get one with one more array dimension.</summary>
    private const int ARRAY_OF = +1 << DIM_SHIFT;

    /// <summary>The constant to be added to an abstract type to get one with one less array dimension.</summary>
    ...
    private const int FORWARD_UNINITIALIZED_KIND = 4 << KIND_SHIFT;
    private const int LOCAL_KIND = 5 << KIND_SHIFT;
    private const int STACK_KIND = 6 << KIND_SHIFT;
    ...
    /// <summary>The basic block to which these input and output stack map frames correspond.</summary>
    internal Label owner;

    /// <summary>The input stack map frame locals. This is an array of abstract types.</summary>
    private int[] inputLocals;

    /// <summary>The input stack map frame stack. This is an array of abstract types.</summary>
    private int[] inputStack;

    /// <summary>The output stack map frame locals. This is an array of abstract types.</summary>
    private int[] outputLocals;

    /// <summary>The output stack map frame stack. This is an array of abstract types.</summary>
    ...
    /// <summary>The index of the top stack element in <see cref="outputStack"/>.</summary>
    private short outputStackTop;

    /// <summary>The number of types that are initialized in the basic block. See <see cref="initializations"/>.</summary>
    ...
    /// <c>MethodVisitor.visitFrame</c>, i.e. either <see cref="Opcodes.TOP"/>, <see cref="Opcodes.INTEGER"/>,
    /// <see cref="Opcodes.FLOAT"/>, <see cref="Opcodes.LONG"/>, <see cref="Opcodes.DOUBLE"/>,
    /// <see cref="Opcodes.NULL"/>, or <see cref="Opcodes.UNINITIALIZED_THIS"/>, or the internal name of a
    /// class, or a Label designating a NEW instruction (for uninitialized types).</param>
    ...
    private static int GetAbstractTypeFromDescriptor(SymbolTable symbolTable, string buffer, int offset)
    {
        ...
        switch (buffer[offset])
        {
            ...
            case 'D':
                return DOUBLE;
            case 'L':
                internalName = buffer.Substring(offset + 1, buffer.Length - offset - 2);
                return REFERENCE_KIND | symbolTable.AddType(internalName);
            case '[':
                ...
                switch (buffer[elementDescriptorOffset])
                {
                    ...
                    case 'D':
                        ...
                        break;
                    case 'L':
                        internalName = buffer.Substring(elementDescriptorOffset + 1, buffer.Length - elementDescriptorOffset - 2);
            ...
            default:
                throw new ArgumentException("Invalid descriptor: " + buffer.Substring(offset));
        }
    }
    ...
    internal void SetInputFrameFromDescriptor(SymbolTable symbolTable, int access, string descriptor, int maxLocals)
    {
        ...
        foreach (Type argumentType in Type.GetArgumentTypes(descriptor))
        {
            int abstractType =
    ...
    /// <param name="local">the local variable types, described using the same format as in
    /// <c>MethodVisitor.visitFrame</c>.</param>
    /// <param name="numStack">the number of operand stack elements.</param>
    /// <param name="stack">the operand stack types, described using the same format as in
    /// <c>MethodVisitor.visitFrame</c>.</param>
    internal void SetInputFrameFromApiFormat(SymbolTable symbolTable, int numLocal, object[] local, int numStack, object[] stack)
    {
        int inputLocalIndex = 0;
        for (int i = 0; i < numLocal; ++i)
        {
            inputLocals[inputLocalIndex++] = GetAbstractTypeFromApiFormat(symbolTable, local[i]);
            if (local[i] is int intValue && (intValue == Opcodes.LONG || intValue == Opcodes.DOUBLE))
            {
                inputLocals[inputLocalIndex++] = TOP;
            }
        }
        ...
        for (int i = 0; i < numStack; ++i)
        {
            ...
            if (stack[i] is int intValue2 && (intValue2 == Opcodes.LONG || intValue2 == Opcodes.DOUBLE))
            {
    ...
    /// <summary>Returns the size of the input stack of this frame.</summary>
    /// <returns>the size of the input stack of this frame.</returns>
    ...
    private void push(SymbolTable symbolTable, string descriptor)
    {
        int typeDescriptorOffset =
            descriptor[0] == '(' ? Type.GetReturnTypeOffset(descriptor) : 0;
    ...
    private void pop(int elements)
    {
        ...
        {
            outputStackTop = (short)(outputStackTop - elements);
        }
        else
        {
            // If the number of elements to be popped is greater than the number of elements in the output
            // stack, clear it, and pop the remaining elements from the input stack.
            outputStackStart = (short)(outputStackStart - (elements - outputStackTop));
            outputStackTop = 0;
        }
    }
    ...
    /// <param name="symbolTable">the type table to use to lookup and store type <see cref="Symbol"/>.</param>
    internal void Execute(int opcode, int arg, Symbol argSymbol, SymbolTable symbolTable)
    {
    ...
    /// </summary>
    /// <param name="symbolTable">the type table to use to lookup and store type <see cref="Symbol"/>.</param>
    /// <param name="dstFrame">the <see cref="Frame"/> whose input frame must be updated. This should be
    /// the frame of a successor, in the control flow graph, of the basic block corresponding to this
    /// frame.</param>
    /// <param name="catchTypeIndex">if 'frame' corresponds to an exception handler basic block, the
    /// type table index of the caught exception type, otherwise 0.</param>
    ...
    /// </summary>
    /// <param name="symbolTable">the type table to use to lookup and store type <see cref="Symbol"/>.</param>
    /// <param name="sourceType">the abstract type with which the abstract type array element must be
    /// merged. This type should be of CONSTANT_KIND, REFERENCE_KIND, UNINITIALIZED_KIND or
    /// FORWARD_UNINITIALIZED_KIND kind, with positive or <c>null</c> array dimensions.</param>
    /// <param name="dstTypes">an array of abstract types. These types should be of CONSTANT_KIND,
    /// REFERENCE_KIND, UNINITIALIZED_KIND or FORWARD_UNINITIALIZED_KIND kind, with positive or
    /// <c>null</c> array dimensions.</param>
    ...
    /// <summary>
    /// Makes the given MethodWriter visit the input frame of this <see cref="Frame"/>. The visit is
    /// done with the <c>MethodWriter.visitFrameStart</c>, <c>MethodWriter.visitAbstractType</c> and
    /// <c>MethodWriter.visitFrameEnd</c> methods.
    /// </summary>
    /// <param name="methodWriter">the MethodWriter that should visit the input frame of this
    /// <see cref="Frame"/>.</param>
    ...
    /// </summary>
    /// <param name="symbolTable">the type table to use to lookup and store type <see cref="Symbol"/>.</param>
    /// <param name="abstractType">an abstract type, restricted to <see cref="CONSTANT_KIND"/>,
    /// <see cref="REFERENCE_KIND"/>, <see cref="UNINITIALIZED_KIND"/> or
    /// <see cref="FORWARD_UNINITIALIZED_KIND"/> types.</param>
    /// <param name="output">where the abstract type must be put.</param>
    /// <seealso cref="https://docs.oracle.com/javase/specs/jvms/se9/html/jvms-4.html#jvms-4.7.4">JVMS 4.7.4</seealso>
